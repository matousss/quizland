import React, {AnimationEventHandler, ComponentProps, ElementType, useRef, useState} from "react";
...
const AnimationCard: FC<{ success: boolean } & ComponentProps<any>> = ({success, ...props}) => (
    ...
    >
        <div className={'m-auto'}>
            {/*{success ? '✓' : '✗'}*/}
            {success ? <CheckIcon className={'w-28'}/> : <XMarkIcon className={'w-28'}/>}
        </div>
    </div>
)
...
const CardFace: FC<{ show: boolean } & PropsWithChildren> = ({children, show}) => (
    <Transition
        ...
        className={'border border-middle flex py-6 bg-secondary absolute z-10 w-full h-full rounded-lg transition-transform duration-500 backface-hidden font-sans'}
    >
        <div
...
const FlipCard: FC<{
    ...
    return (
        <div className={'w-[18rem] sm:w-[33rem] md:w-[39rem] z-[1] select-none'}>
